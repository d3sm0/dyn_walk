...
for i in range(1000):
    ...
    #In this part I restart the same controller at different times after the robot makes a complete gait and
    #comes back in a quite stable equilibrium
    """
    if(i>5.7):
        i-=5.
        T=2
        observation, reward, done, info = env.step(np.array(input(w_best,i)))
       # print("5.6")
    ""
    elif(i>5.5):
        i-=5.5
        T=2
        observation, reward, done, info = env.step(np.array(input(w,i)))
      #  print("5.6")
    """
    if(i>4.6):
        i-=4.2
