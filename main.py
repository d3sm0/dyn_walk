...
def main():
    ...
    env_dims = (main_env.observation_space.shape[0], main_env.action_space.shape[0], main_env.action_space.high)
    workers = []
    threads = []
    ...
    with tf.Session() as sess:
        ...
        save_th = threading.Thread(target=cont_save(SAVE_EVERY, target, coord, saver, writer))
        save_th.start()
        # save_th.join()
        coord.join(threads)
