...
def main():
    ...
    pool = multiprocessing.Pool(initializer=init, initargs=(lk,))
    pool.map(target, iterable)
    pool.close()
    pool.join()
    """

    target_update = threading.Lock()
    # TODO fix this
    # to use a stochastic policy, set stochastic = True, and remove exploration noise from
    # train.py, line 52
    target = Agent('target', env_dim=env_dims, target_network_lock=target_update, h_size=H_SIZE, stochastic=True)
    ...
    for i in range(CPU_CORE - 1):
        agent = Agent('worker_{}'.format(i), env_dim=env_dims, target=target, h_size=H_SIZE, writer=writer,
                      target_network_lock=target_update, stochastic=True)
        env = gym.make(ENV_NAME)
