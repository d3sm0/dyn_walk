...
from models.networks import encoder, decoder, recurrent_encoder


class VAE:
    def __init__(self, obs_dim, acts_dim, z_dim, batch_size, is_recurrent=False):
        """
    ...
    def _build_graph(self, is_recurrent=False):
        """
        ...
        with tf.variable_scope('vae'):
            ...
            if is_recurrent:
                self.mean, self.sd = recurrent_encoder(x, self.z_dim)
            else:
                self.mean, self.sd = encoder(x, self.z_dim)
            self.z = self._sample()
