...
class Actor(object):
    def __init__(self, obs_space, action_space, action_bound, h_size, lr=1e-4, act=tf.nn.elu, stochastic=False):
        ...
        self.action_grads = tf.placeholder('float32', shape=[None, action_space], name='action_grads')

        self.mu_hat = self.build_network(h_size, action_space, action_bound, act, stochastic=stochastic)

        self.actor_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=tf.get_variable_scope().name)

        self.actor_grads = tf.gradients(self.mu_hat, self.actor_params, -self.action_grads)
