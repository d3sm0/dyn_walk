...
NORMALIZE = True
DESCRIPTON = 'Testing osim concatenatig 3 frames with with augmented reward and time. Using prioritzed memory and 128,64 as size of hidden layer'
...
def main():
    ...
    if ENV_NAME == 'osim':
        ...
        try:
            env = EnvWrapper(RunEnv, augment_rw=USE_RW, add_time=True, concat=CONCATENATE_FRAMES, normalize=NORMALIZE, add_acceleration = 5)
    ...
    target = Agent( name='target' , env_dims=env_dims , h_size=H_SIZE , policy = POLICY, act=ACTIVATION, split_obs = None) #env sep
